...
func (r Repo) MRURL(branch string, body string) string {
	...
	if strings.Contains(host, "gitlab") {
		u := fmt.Sprintf("https://%s/%s/%s/-/merge_requests/new?merge_request[source_branch]=%s", host, owner, repo, url.QueryEscape(branch))
		if body != "" {
...
func parseUnifiedDiff(raw string) map[string]models.DiffFile {
	...
	for sc.Scan() {
		...
		case strings.HasPrefix(line, "+"):
			n := newLine
			hunk.Lines = append(hunk.Lines, models.DiffLine{Type: "add", Number: &n, Content: strings.TrimPrefix(line, "+")})
			newLine++
		case strings.HasPrefix(line, "-"):
			hunk.Lines = append(hunk.Lines, models.DiffLine{Type: "remove", Number: nil, Content: strings.TrimPrefix(line, "-")})
		default:
			n := newLine
			hunk.Lines = append(hunk.Lines, models.DiffLine{Type: "context", Number: &n, Content: strings.TrimPrefix(line, " ")})
			newLine++
		}
		result[current.Path] = *current
	}
