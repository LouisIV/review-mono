...
func (s Store) UpsertSession(session models.Session) error {
	...
	if err := os.MkdirAll(filepath.Join(s.Root, "sessions", session.ID), 0o700); err != nil {
		return err
	}
...
func (s Store) SaveComments(sessionID string, comments []models.Comment) error {
	err := os.MkdirAll(filepath.Join(s.Root, "sessions", sessionID), 0o700)
	if err != nil {
		return err
	}
...
func (s Store) Description(sessionID string) (models.Description, error) {
	...
	//nolint:gosec // Path is scoped under the repo-local review store.
...
func readJSON(path string, target any) error {
	//nolint:gosec // Path is generated by store methods, not user input.
