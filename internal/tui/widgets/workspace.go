...
func NewWorkspace(props Props) Workspace {
	leftWidth := 32
	rightWidth := max(props.Width-leftWidth-3, 40)
...
func (w Workspace) View() string {
	...
	header := titleStyle.Render("feature/tui-widgets -> main") +
		mutedStyle.Render("  in_review  3 commits  5 files  4 open comments")
